      ...
      <div className="w-full h-auto p-2 pb-14">
        <div className={`w-full h-auto grid  ${trackData?.order_info?"grid-rows-3 sm:grid-rows-2 md:grid-rows-1 sm:grid-cols-2  md:grid-cols-3":"grid-rows-1 grid-cols-1"} relative`}>
          <div className={ trackData?.order_info? "sm:col-span-2 sm:row-span-2 md:col-span-1 md:row-span-1": ""}>
            <div className="text-lg font-semibold">Trak Id: {trackData?.track_info?.tracking_number}</div>
            <div className="w-full h-auto capitalize flex justify-left align-middle items-center gap-4 font-semibold text-base text-gray-700">
              <h1 className="">{trackData?.order_info? trackData?.order_info?.parcel?.from?.country.toUpperCase() : trackData?.track_info?.own_tracking_info?.courier_tracking[0]?.origin_city}</h1> TO{" "}
              <h1>{trackData?.order_info ? trackData?.order_info?.parcel?.to?.country.toUpperCase() : trackData?.track_info?.own_tracking_info?.courier_tracking[0]?.destination_city}</h1>
            </div>
            <div className="w-full h-auto py-3">
              <Image width={100}height={100}className="w-[100px] h-[100px]"src={"/Box.png"}alt="Box Icon"/>
            </div>
          </div>
          {trackData?.order_info ? (
            <>
              <div className={`w-full h-auto flex flex-col gap-1 text-sm`}>
                ...
                <div>From : {trackData?.order_info?.parcel?.sender.address}</div>
              ...
              <div className={`w-full h-auto flex flex-col gap-1  text-sm`}>
                ...
                <div>Phone : {trackData?.order_info?.parcel?.reciver.phone}</div>
                <div>Email : {trackData?.order_info?.parcel?.reciver.email}</div>
                ...
                <div className="w-full lg:h-[100px] overflow-auto scrollbar">
                  <Table className="w-auto">
                    <TableBody className="w-auto">
                      {trackData?.order_info?.parcel?.item?.list.map((item, index) => (
                        <TableRow key={index} className="w-auto">
                            <TableCell className="w-auto">{index + 1}</TableCell>
            ...
            <div className="w-full h-auto flex flex-col gap-3 ">
              ...
              {[...(trackData?.track_info?.own_tracking_info?.courier_tracking && trackData?.track_info?.own_tracking_info?.courier_tracking?.length >
                ...
                <div key={i} className=" flex sm:gap-2 flex-col sm:flex-row font-medium text-base item-center">
                  ...
                  <h1 className="sm:w-[230px] text-right">{" - "} {replaceCourierWithFinex(item?.location)}</h1>
                </div>
              ))}
            </div>
          </div>
