...
const AdminDashBoardOrderCreateSection = () => {
    const [priceChart, setPriceChart] = useState([])
    const [selectedDestination,setSeletedDestination] = useState({from:"",to:"",priceID:""})

    const router = useRouter()

    const createHandler =()=>{
if(selectedDestination.from && selectedDestination.to&& selectedDestination.priceID){
    router.push(`/admin/order/${selectedDestination.priceID}`)
}else{
        ...
        <div className="w-full h-auto flex-col sm:flex-row  flex gap-3 items-end align-middle justify-center pb-48">
          <div className="w-full sm:flex-row flex-col flex gap-2">
            <SelecteSearchBox
              ...
              setValue={(e)=>{setSeletedDestination({from:e.from,to:e.to,priceID:e.id})}}
            />
...
export default AdminDashBoardOrderCreateSection;
