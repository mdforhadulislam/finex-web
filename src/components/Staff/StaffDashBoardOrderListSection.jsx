...
import { getRequestSend, ORDER_API, SINGLE_PRICE_LIST } from '@/data/ApiMethod';
import InputBox from '@/utils/InputBox';
import { useRouter } from 'next/router';
import { useContext, useEffect, useState } from 'react';
import { FiEdit } from 'react-icons/fi';
import { IoEyeOutline } from 'react-icons/io5';
import StaffDashBoardOrderCreatedPopup from './StaffDashBoardOrderCreatedPopup';
import StaffDashBoardOrderListBox from './StaffDashBoardOrderListBox';

const StaffDashBoardOrderListSection  = () => {
    ...
    return (
      <div className="w-full h-auto py-2" id="allOrder">
          ...
          <div className="w-full flex align-middle items-center flex-col mt-3 p-3 gap-3">
            {orderData?.map((item, index) => (
              <div className="w-full h-auto relative" key={index}>
                <StaffDashBoardOrderListBox orderData={item} />
        ...
        <StaffDashBoardOrderCreatedPopup orderData={viewOrderData} />
      </div>
    );
  };
  

export default StaffDashBoardOrderListSection