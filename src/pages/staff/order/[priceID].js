import StaffDashBoardOrderCreatedPopup from "@/components/Staff/StaffDashBoardOrderCreatedPopup";
...
const StaffOrderCreate = () => {
  ...
  const updateHandler = () => {
    loading.loadingStart();
    putRequestSend(SINGLE_ORDER_API(router.query.trackID), {}, parcel).then(
      (res) => {
        if (res.status == 200) {
    ...
    if (router.query.trackID) {
      getRequestSend(SINGLE_ORDER_API(router.query.trackID)).then((res) => {
        if (res.status == 200) {
          ...
          toast.success(
            "Only Updated Parcel related Data (Sender Details,Reciver Details)"
          );
        }
      });
    }
  ...
  return (
    <div className="w-full h-auto py-2" id="createorder">
              ...
              <div className="w-full h-auto 5st-box-style">
                ...
                <div className="w-full h-auto grid grid-cols-1 grid-rows-2 sm:grid-cols-2 sm:grid-rows-1 gap-3 pb-3">
                  ...
                  <InputBox
                    ...
                    action={(e) => {
                      setPayment({ ...payment, pExtraCharge: Number(e.target.value) });
                    }}
                  />
                  ...
                  <InputBox
                    ...
                    action={(e) => {
                      setPayment({ ...payment, pDiscount: Number(e.target.value) });
                    }}
                  />
                  ...
                  <InputBox
                    ...
                    action={(e) => {
                      setPayment({ ...payment, pRecived: Number(e.target.value) });
                    }}
                  />
                </div>
              </div>
      ...
      <StaffDashBoardOrderCreatedPopup orderData={getOrderfullData} />
    </div>
  );
};

export default StaffOrderCreate;
